...
class WordFreq {
    ...
    std::map<std::string,int64_t> wordCountMap;
    ...
    const int64_t getWordFreq(const std::string& word) const {
    ...
    int64_t getDotProduct(const WordFreq& wordFreq) {
        int64_t acc = 0;
        for (std::map<std::string,int64_t>::iterator it = wordCountMap.begin();
                it != wordCountMap.end(); it++) {
...
int main(int argc, char* argv[]) {
    ...
    int64_t t1 = file1Words.getDotProduct(file2Words);
